    ...
    <div className="mx-auto max-w-[210mm] bg-white p-8 print:p-0">
      ...
      <div className="report-container">
        ...
        <div className="mb-6 border-b-2 border-black pb-4 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Work Order #{id.slice(0, 8)}</p>
        </div>
        ...
        <div className="mb-6">
          ...
          {completedCards.length === 0 ? (
            <p className="text-sm text-muted-foreground">No completed job cards.</p>
          ) : (
            completedCards.map((jc) => {
              const jcMinutes = jc.entries?.reduce((s, e) => s + e.duration_minutes, 0) || 0
              const jcParts = jc.parts?.length || 0
              return (
                <div key={jc.id} className="mb-4 rounded border bg-muted p-3 text-sm">
                  ...
                  {jc.unresolved_issues && (
                    <div className="mt-1 text-sm text-danger-strong"><strong>Unresolved:</strong> {jc.unresolved_issues}</div>
                  )}
                </div>
              )
            })
        ...
        <div className="mt-10 flex justify-between border-t pt-4 text-sm">
          <div className="text-center">
            ...
            <span className="text-xs text-muted-foreground">Technician Signature</span>
          </div>
          <div className="text-center">
            <div className="mb-1 w-40 border-b border-black">&nbsp;</div>
            <span className="text-xs text-muted-foreground">Supervisor Signature</span>
          </div>
        </div>

        {/* Footer */}
        <div className="mt-8 border-t pt-2 text-center text-[10px] text-muted-foreground">
